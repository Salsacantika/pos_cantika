...
@section('title', 'Riwayat Penjualan Tahunan')

@section('content')
<div class="container-fluid py-4 px-lg-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold" style="color: #6e4631;"><i class="bi bi-calendar-range me-2"></i>Riwayat Penjualan Tahunan</h2>
            <p class="text-muted small mb-0">Daftar transaksi berdasarkan tahun tertentu.</p>
        </div>
    ...
    <div class="card border-0 shadow-sm p-4 mb-4 rounded-4">
        <form action="{{ route('penjualan.riwayat.tahunan') }}" method="GET" class="row g-3 align-items-center">
            <div class="col-md-4">
                <label class="form-label fw-semibold small">Pilih Tahun</label>
                <input type="number" name="tahun" value="{{ $tahun }}" min="2000" max="2100" class="form-control rounded-3" placeholder="Contoh: 2026">
            </div>
                ...
                <tbody>
                    @forelse($sales as $sale)
                        <tr>
                            ...
                            <td>{{ $sale->created_at->format('d M Y, H:i') }} WIB</td>
                    ...
                    @empty
                        <tr><td colspan="5" class="text-center py-4 text-muted">Tidak ada transaksi pada tahun ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
